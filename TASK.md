...
状态：**已验收（2026-09-19，用户确认）**。本文件即将按 2A/2B 惯例转入 2C Checklist §5，随后改写为阶段 3 锚点。
验收依据：T1–T6 全部落地，上下文包缓存有真实调用方且 AC1 已在真实服务上按"同一会话
hit→仍拒绝"验证，三条 runtime 路径 usage 归一化已接，命中率经 ops 指标可读，四门禁 + E2E 全绿。
验收时决定：priceVersion 来源与真实付费模型抽样归阶段 6「形成质量与成本报告」；
文件/摘要层不套缓存、CLI 能力声明不记录、跨进程 single-flight 延后（理由见 tasks 文档）。
