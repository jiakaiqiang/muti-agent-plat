...
const forbiddenPatterns = [
  ...
  { pattern: /@create-agent="createAgent"|showAgentForm|agent-create-form/, reason: '会话右侧 Agent 列表不应再显示添加 Agent 入口' }
];
...
for (const label of requiredChineseLabels) {
  if (!workspaceSource.includes(label) && !agentPanelSource.includes(label) && !agentManagerSource.includes(label)) {
