...
async function waitForEventCount(apiBase, sessionId, count, timeoutMs = 15_000) {
  const deadline = Date.now() + timeoutMs;
  let last = 0;
  while (Date.now() < deadline) {
    const events = await api(apiBase, `/sessions/${sessionId}/events`);
    last = events.data.items.length;
    if (last >= count) {
  ...
  throw new Error(`Timed out waiting for ${count} persisted events, last=${last}`);
}
