...
export class BrokerGateway {
  ...
  refreshWorkspace(client: BrokerClient, input: RegisterWorkspaceInput): BrokerWorkspaceRegistration {
    const existing = this.registrations.get(input.workspaceId);
    if (!existing || existing.clientId !== input.clientId) {
      throw new Error(`workspaceId is not registered by this client: ${input.workspaceId}`);
    }
    const registration: BrokerWorkspaceRegistration = {
      ...existing,
      providerKind: input.providerKind,
  ...
  dispatch(request: WorkspaceOperationRequest): void {
    ...
    if (registration && registration.dataEpoch !== this.currentDataEpoch()) {
      throw new Error(`STALE_DATA_EPOCH: broker workspace ${request.workspaceId} belongs to ${registration.dataEpoch}.`);
    }
