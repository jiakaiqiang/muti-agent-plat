...
import CollaborationGraphView from '@/components/CollaborationGraphView.vue'
import DebugRuntimeView from '@/components/DebugRuntimeView.vue'
import FileRevisionCandidateEditor from '@/components/FileRevisionCandidateEditor.vue'
import SessionSidebar from './SessionSidebar.vue'
import UiIcon from '@/components/UiIcon.vue'
import UserInputBox from '@/components/UserInputBox.vue'
import RuntimeStopStatePanel from '@/components/RuntimeStopStatePanel.vue'
...
import SelectedWorkflowGraph from './workspace/SelectedWorkflowGraph.vue'
import WorkflowExecutionDetails from './workspace/WorkflowExecutionDetails.vue'
import WorkflowCatalog from './workspace/WorkflowCatalog.vue'
import HistoricalDiffDialog from './workspace/HistoricalDiffDialog.vue'
import './workspace/task-workspace.css'
import { findLatestBriefPayload } from '@/components/briefEventPayload'
import { resolveSessionWorkspacePostReviewAction } from '@/components/session-workspace-post-review-action'
...
const taskWorkspace = useTaskWorkspaceStore()
const historyDiff = useHistoryDiffStore()
const showCatalog = ref(false)
const catalogBusy = ref(false)
const leftOpen = ref(false)
const rightOpen = ref(false)
const savedViews = new Map<string, SessionViewMode>()
...
const viewModes: SessionViewMode[] = ['chat', 'workflow', 'workflow_details']

function isViewMode(value: string | null): value is SessionViewMode {
  return value === 'chat' || value === 'collaboration_graph' || value === 'workflow' || value === 'workflow_details' || value === 'debug'
}
...
async function switchWorkspaceView(mode: SessionViewMode) {
  ...
  if (currentSessionId.value) savedViews.set(currentSessionId.value, mode)
  if (mode !== 'workflow') taskWorkspace.select({ nodeId: '', taskId: '', nodeRunId: '' })
  sessionStore.switchViewMode(mode)
  if (mode === 'chat') {
    await nextTick()
    chatTimeline.value?.restoreReadingPosition()
  }
...
onMounted(async () => {
  ...
  // Cached ahead of time so waking the local helper costs no round-trip once the
  // user clicks, keeping the protocol launch inside the browser's activation window.
...
// 与 apps/web 同步：后端探测不可达与会话被中断是两件事，前者禁用输入，后者放开。
...
const hasWorkflow = computed(() => taskWorkspace.runs.length > 0 || Boolean(sessionStore.currentSession?.workflowRunId || sessionStore.currentSession?.workflowRun))
const chatTimeline = ref<InstanceType<typeof ChatTimeline>>()
let historyRefresh: ReturnType<typeof setTimeout> | undefined
watch(currentSessionId, (id, previousId) => {
  workspaceUiStore.activateTask(id)
  historyDiff.close()
  if (previousId) showCatalog.value = false
  void taskWorkspace.load(id)
}, { immediate: true })
watch(() => events.value.at(-1)?.id, () => {
  const latest = events.value.at(-1)
  if (!latest || !/^(workflow_|task_|artifact_|session_status_changed|final_delivery_created)/.test(latest.type)) return
  clearTimeout(historyRefresh)
  historyRefresh = setTimeout(() => void taskWorkspace.load(currentSessionId.value), 300)
})
onBeforeUnmount(() => clearTimeout(historyRefresh))
async function goToConfirmation(id?: string) {
  await switchWorkspaceView('chat')
  await nextTick()
  chatTimeline.value?.focusConfirmation(id ?? activeConfirmation.value?.confirmationId ?? '')
}
async function useCatalogWorkflow(workflowId: string, version: number) {
  if (catalogBusy.value || activeConfirmation.value?.reason !== 'select_workflow') return
  catalogBusy.value = true
  try {
    await resolveConfirmation(`workflow:${workflowId}:${version}`)
    if (sessionStore.currentSession?.status !== 'WAIT_WORKFLOW_SELECT') showCatalog.value = false
...
async function openWebWorkflowManager() {
  workflowMappingRejection.value = undefined
  try {
    await window.agentClusterDesktop?.openWebWorkflowManager()
  } catch (error) {
    showMessage(error instanceof Error ? error.message : '无法打开 Web 流程管理，请检查平台连接。', 'error')
  }
}
watch(
...
useWorkspaceSync({ syncEvents: syncSessionEventConnection, refreshDetails: id => taskWorkspace.load(id) })
  ...
  try {
    ...
    if (navigate) {
      await router.push({
        name: 'workspace-session',
        params: { sessionId },
        query: { ...route.query, view: savedViews.get(sessionId) ?? 'chat' }
      })
    }
  } catch (error) {
...
<template>
  <div :class="['workspace-shell', 'task-workspace', `mode-${currentMode}`, { 'left-open': leftOpen, 'right-open': rightOpen, 'catalog-open': showCatalog }]">
    <HistoricalDiffDialog />
    <teleport to="body">
    ...
    <section class="workspace-main" :class="{ 'has-workflow-tabs': hasWorkflow }">
      <WorkflowCatalog v-if="showCatalog" :can-use="activeConfirmation?.reason === 'select_workflow' && derivedStatus === 'WAIT_WORKFLOW_SELECT'" :busy="catalogBusy" :task-title="workspaceLabel" @use="useCatalogWorkflow" @close="showCatalog = false" />
      <template v-else>
      <header class="workspace-header">
        <button class="workspace-panel-toggle" type="button" @click="leftOpen = !leftOpen" aria-label="切换任务列表">任务列表</button>
        ...
        <button class="workspace-panel-toggle" type="button" @click="rightOpen = !rightOpen" aria-label="切换右侧信息">右侧信息</button>
      </header>

      <nav v-if="hasWorkflow" class="workspace-view-tabs" role="tablist" aria-label="任务视图">
        <button v-for="mode in viewModes" :key="mode" type="button" role="tab" :aria-selected="currentMode === mode" :class="{ active: currentMode === mode }" @click="switchWorkspaceView(mode)"><UiIcon :name="viewModeIcon(mode)" :size="16" />{{ viewModeLabel(mode) }}<span v-if="mode === 'chat' && pendingMessages.length" class="pending-count">{{ pendingMessages.length }}</span></button>
      </nav>
      ...
      <div :class="['workspace-content', {
        ...
        <div v-show="currentMode === 'chat'" class="chat-pane">
        ...
        />
        <section v-else-if="currentMode === 'workflow' || currentMode === 'workflow_details'" class="workflow-run-surface">
          <header class="run-toolbar"><select :value="taskWorkspace.selection.runId" aria-label="历史工作流运行" @change="taskWorkspace.select({ runId: ($event.target as HTMLSelectElement).value, nodeId: '', taskId: '', nodeRunId: '' })"><option v-for="run in taskWorkspace.runs" :key="run.id" :value="run.id">{{ run.workflowName }} · v{{ run.workflowVersion }} · {{ run.createdAt }}</option></select><el-button v-if="pendingMessages.length" size="small" @click="goToConfirmation()">去处理（{{ pendingMessages.length }}）</el-button><el-button size="small" @click="taskWorkspace.load(currentSessionId)">刷新</el-button></header>
          <el-alert v-if="taskWorkspace.error" :title="taskWorkspace.error" type="error" :closable="false" />
          <p v-if="taskWorkspace.loading && !taskWorkspace.detail" role="status">正在读取执行记录…</p>
          <template v-else-if="taskWorkspace.detail"><SelectedWorkflowGraph v-if="currentMode === 'workflow'" :definition="taskWorkspace.detail.run.definitionSnapshot" :node-runs="taskWorkspace.detail.nodeRuns" :selected-node-id="taskWorkspace.selection.nodeId" @select="taskWorkspace.select({ nodeId: $event, taskId: '', nodeRunId: '' }); rightOpen = true" /><WorkflowExecutionDetails v-else :events="events" @process="goToConfirmation" @locate="sessionStore.switchViewMode('workflow'); rightOpen = true" /></template>
          <el-empty v-else-if="!taskWorkspace.loading" description="暂无可用的流程快照" />
        </section>
        <DebugRuntimeView
          v-else-if="currentMode === 'debug'"
          :session-id="currentSessionId"
          :events="events"
        />
      </div>
    ...
    <aside v-if="currentMode !== 'chat' && !showCatalog" class="workspace-context-panel">
      <WorkflowExecutionDetails v-if="currentMode === 'workflow' && taskWorkspace.selection.nodeId" :events="events" inspector @close="taskWorkspace.select({ nodeId: '', taskId: '', nodeRunId: '' })" @process="goToConfirmation" />
      <template v-else><header class="panel-header">群聊消息 · 只读</header><ChatTimeline :messages="messages" :session-id="currentSessionId" :status="derivedStatus" :disconnected="backendUnreachable" read-only @go-to-confirmation="goToConfirmation" /></template>
    </aside>
    <AgentStatusPanel
    ...
    <section v-if="workflowMappingRejection" class="modal-backdrop" aria-label="工作流未启动">
      <div class="modal-panel workflow-mapping-result-dialog" role="dialog" aria-modal="true">
        ...
        <footer class="confirmation-card__actions"><button type="button" class="action-button default" @click="workflowMappingRejection = undefined; suppressWorkflowAutoOpen = false; showCatalog = true">选择其他已发布流程</button><button type="button" class="action-button primary" @click="openWebWorkflowManager">前往 Web 创建新流程</button></footer>
      </div>
    </section>
