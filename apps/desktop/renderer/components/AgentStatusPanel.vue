...
<template>
  <aside class="agent-panel">
    ...
    <ConfirmationCard
      v-if="activeConfirmation && !readOnly"
    ...
    <section class="panel-section agent-list-section">
      ...
      <article v-for="(agent, index) in agents" :key="agent.agentId" class="agent-card">
        ...
        <div class="agent-card__body">
          ...
          <p class="agent-progress-text">{{ tasks.filter(task => task.assignee?.id === agent.agentId && task.status === 'completed').length }} / {{ tasks.filter(task => task.assignee?.id === agent.agentId).length }} 个任务已完成</p>
        ...
        <div>
          ...
          <ul v-if="task.artifacts.length" class="task-artifacts">
            <li v-for="artifact in task.artifacts" :key="artifact.artifactId">
              <button type="button" class="file-diff-link" @click="sessionStore.currentSession && historyDiff.openArtifact(artifact.artifactId, sessionStore.currentSession.id)">{{ artifact.title }}</button>
