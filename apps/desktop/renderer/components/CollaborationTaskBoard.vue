...
import { summarizeContextSupplement } from '@/components/contextSupplementSummary'
      ...
      <div class="task-board-body">
          ...
          <div class="task-board-task-list">
            <article v-for="task in tasks" :key="task.taskId" class="task-board-task">
              ...
              <footer v-if="task.artifacts.length">
                <button type="button" class="file-diff-link" v-for="artifact in task.artifacts.slice(0, 3)" :key="artifact.artifactId" @click="sessionStore.currentSession && historyDiff.openArtifact(artifact.artifactId, sessionStore.currentSession.id)">
        ...
        <section class="task-board-delivery" aria-label="Review & Delivery">
          ...
          <div v-if="deliveryEvidence.length" class="task-board-evidence-list" aria-label="Delivery Evidence">
            <strong>交付证据</strong>
            <button type="button" class="file-diff-link" v-for="artifact in deliveryEvidence" :key="artifact.id" @click="sessionStore.currentSession && historyDiff.openDeliveryArtifact(artifact.id, sessionStore.currentSession.id)">
