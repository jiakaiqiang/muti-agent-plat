...
import WorkflowSelectionDialog, { type WorkflowSelectionOption } from '@/components/workflow/WorkflowSelectionDialog.vue'
...
const availableWorkflows = computed<WorkflowSelectionOption[]>(() => {
  if (taskWorkspace.catalog.length) {
    return taskWorkspace.catalog
      .map((workflow) => ({
...
onMounted(async () => {
  ...
  try {
    await taskWorkspace.loadCatalog()
  } catch {
