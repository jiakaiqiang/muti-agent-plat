...
import ChatScrollArea from '@/components/ChatScrollArea.vue'
...
import CapabilityApprovalCard from '@/components/CapabilityApprovalCard.vue'
import { observedArtifactFileChanges, platformArtifactProjections } from '@/components/artifactFileChangeModel'
...
const emit = defineEmits<{
  ...
  goToConfirmation: [confirmationId: string]
}>()

const agentStore = useAgentStore()
const timeline = computed(() => collapseDuplicateFailureMessages(props.messages))
const scrollArea = ref<InstanceType<typeof ChatScrollArea>>()
const diff = useHistoryDiffStore()
function readingKey() {
  const sessionId = props.sessionId ?? props.messages[0]?.sessionId
  return sessionId ? `task-reading:${sessionId}:${props.readOnly ? 'side' : 'center'}` : ''
}
function restoreReadingPosition() {
  return scrollArea.value?.restoreReadingPosition() ?? false
}
function focusConfirmation(id: string) {
  const target = Array.from(scrollArea.value?.element?.querySelectorAll<HTMLElement>('[data-confirmation-id]') ?? []).find(el => el.dataset.confirmationId === id)
  target?.scrollIntoView({ block: 'center', behavior: 'smooth' })
  target?.focus({ preventScroll: true })
}
defineExpose({ focusConfirmation, restoreReadingPosition })
function openFile(message: ChatMessage, path: string, projection = false) {
  const payload = message.payload as ArtifactEventPayload
  const files = projection
    ? platformArtifactProjections(payload).map(change => ({ path: change.path, operation: 'reference' as const, after: change.content, notice: '平台生成的待写入内容，尚不属于已执行文件差异。' }))
    : filesFromChanges(payload.systemEvidence?.workspaceChangeSet?.changes ?? [])
  diff.open({ files, path, title: payload.title ?? '文件修改', source: `${senderLabel(message)} · ${message.createdAt} · 本轮执行前 → 执行后 · ${payload.systemEvidence?.workspaceChangeSet?.id ?? '平台生成内容'}`, sessionId: message.sessionId })
}

function historicalFiles(message: ChatMessage) {
  return filesFromChanges(artifactPayload(message)?.systemEvidence?.workspaceChangeSet?.changes ?? [])
}

function openReport(message: ChatMessage) {
  const report = projectAnalysisReport(message)
  if (report) diff.open({ files: [{ path: report.label, operation: 'reference', after: report.content, notice: '已保存的报告内容 · 只读引用，不表示工作区发生修改。' }], title: '报告内容', source: `${senderLabel(message)} · ${message.createdAt}`, sessionId: message.sessionId })
}

function openDeliveryReport(message: ChatMessage) {
  const report = deliveryReport(message)
  if (report) diff.open({ files: [{ path: report.suggestedPath, operation: 'reference', after: report.content, notice: '已保存的交付报告 · 只读引用。' }], title: report.title, source: `${senderLabel(message)} · ${message.createdAt}`, sessionId: message.sessionId })
}
...
<template>
  <ChatScrollArea ref="scrollArea" :reading-key="readingKey()" :message-count="messages.length" :status="status" :disconnected="disconnected">
    <article
      v-for="message in timeline"
      :key="message.id"
      class="timeline-item"
      :class="[message.senderType, message.messageType]"
      :data-confirmation-id="message.payload?.confirmationId"
    ...
    >
      ...
      <div class="message-bubble">
        ...
        <section v-if="readOnly && (confirmationFromMessage(message) || capabilityApprovalFromMessage(message))" class="readonly-confirmation"><strong>{{ message.payload?.title ?? '待处理通知' }}</strong><p>{{ message.content }}</p><button v-if="(message.payload?.status ?? 'pending') === 'pending'" type="button" @click="emit('goToConfirmation', String(message.payload?.confirmationId ?? ''))">去处理</button></section>
        <ConfirmationCard
          v-else-if="confirmationFromMessage(message)"
        ...
        <template v-else>
          ...
          <div v-if="message.messageType === 'delivery'" class="structured-block delivery-block">
            ...
            <details v-if="deliveryReport(message)" class="delivery-report" open>
              <summary>
                <span>完整系统架构说明</span>
                <button type="button" class="file-diff-link" @click.stop.prevent="openDeliveryReport(message)">{{ deliveryReport(message)?.suggestedPath }}</button>
              </summary>
          ...
          <div v-if="artifactPayload(message)" class="structured-block artifact-block">
            ...
            <article v-if="projectAnalysisReport(message)" class="project-analysis-report">
              <header>
                ...
                <button type="button" class="file-diff-link" @click="openReport(message)">{{ projectAnalysisReport(message)?.label }}</button>
              </header>
            ...
            <div v-if="artifactPlatformProjections(message).length" class="file-change-list">
              ...
              >
                <header>
                  ...
                  <button type="button" class="file-diff-link" @click="openFile(message, change.path, true)">{{ change.path }}</button>
                </header>
                <p>待写入内容 · 点击文件查看</p>
              </article>
            </div>
            <div v-if="historicalFiles(message).length" class="file-change-list">
              <h4>平台观测的文件变更</h4>
              <article
                v-for="change in historicalFiles(message)"
                :key="`${change.operation}:${change.path}`"
                class="file-change-item"
              >
                <header>
                  <span class="file-operation" :class="change.operation">{{ change.operation === 'move' ? '重命名' : change.operation === 'reference' ? '引用' : fileOperationLabel(change.operation) }}</span>
                  <button type="button" class="file-diff-link" @click="openFile(message, change.path)">{{ change.path }}</button>
                </header>
                <p>已保存的执行版本 · 点击查看差异</p>
              </article>
            </div>
