...
- 验证：执行 Checklist 对应场景，记录命令/环境/结果；失败时保留证据并回到所属任务。
- 承接自阶段 2C（2026-09-19 验收时决定）：(1) `RuntimeUsage.priceVersion` 的实际来源与维护方式
  ——2C 合同已要求"无价格版本则不出具金额"，本任务需定价格表落点（配置文件 / 随
  runtime-model-config / 其他）并接入结算；(2) 真实付费模型抽样跑 `test:e2e:context-bundle-cache`
  同一场景（同一会话缓存命中仍被预算守卫拒绝），需另定模型、数据范围与费用上限（roadmap §8）。
