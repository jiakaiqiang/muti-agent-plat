...
| P2A-AC5 | P2A-T4、P2A-T6 | 长中文、长 schema、图片/工具返回使完整请求超限时，在发送前触发确定的裁剪/容量拒绝；不能只检查 L3。 | **已验收（当前适配器范围）**：请求按 system / 工具 / ContextEnvelope / schema / 工具历史归因并逐轮复算；`token-budget` E2E 验证正常用量记录与极小预算发送前拒绝，T6 长输入矩阵覆盖长中文和长 schema。`generic_llm` 只发送字符串消息，所以多模态在当前适配器上不适用；真实多模态 Provider 未接入、未执行。 |
